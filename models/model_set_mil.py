...
class MIL_Sum_FC_surv(nn.Module):
    def __init__(self, omic_input_dim=16374, fusion=None, size_arg = "small", dropout=0.25, n_classes=4):
        r"""
        Deep Sets Implementation.
        ...
        self.fusion = fusion
        self.size_dict_path = {"small": [384, 256, 256], "big": [1024, 512, 384]}
...
#         self.fusion = fusion
#         self.size_dict_path = {"small": [384, 512, 256], "big": [1024, 512, 384]}
#         self.size_dict_omic = {'small': [256, 256]}
...
#         if self.fusion is not None:
#             hidden = [256, 256]
#             fc_omic = [SNN_Block(omic_input_dim, dim2=hidden[0])]
#             for i, _ in enumerate(hidden[1:]):
#                 fc_omic.append(SNN_Block(dim1=hidden[i], dim2=hidden[i+1], dropout=0.25))
#             self.fc_omic = nn.Sequential(*fc_omic)
...
#         if self.fusion is not None:
#             omic_input_dim = kwargs['x_omic']

#             h_omic = self.fc_omic(omic_input_dim)
#             if self.fusion == 'bilinear':
#                 h_omic=h_omic.view(256)
#                 h = self.mm(h_path.unsqueeze(dim=0), h_omic.unsqueeze(dim=0)).squeeze()
#             elif self.fusion == 'concat':
