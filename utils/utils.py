...
def collate_MIL_survival(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    rad = torch.cat([item[1] for item in batch], dim = 0).type(torch.FloatTensor)
    label = torch.LongTensor([item[2] for item in batch])
    event_time = np.array([item[3] for item in batch])
    c = torch.FloatTensor([item[4] for item in batch])
    return [img, rad, label, event_time, c]

def collate_MIL_survival_rpg(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    rad = torch.cat([item[1] for item in batch], dim = 0).type(torch.FloatTensor)
    omic1 = torch.cat([item[2] for item in batch], dim = 0).type(torch.FloatTensor)
    omic2 = torch.cat([item[3] for item in batch], dim = 0).type(torch.FloatTensor)
    omic3 = torch.cat([item[4] for item in batch], dim = 0).type(torch.FloatTensor)
    omic4 = torch.cat([item[5] for item in batch], dim = 0).type(torch.FloatTensor)
    omic5 = torch.cat([item[6] for item in batch], dim = 0).type(torch.FloatTensor)
    omic6 = torch.cat([item[7] for item in batch], dim = 0).type(torch.FloatTensor)
    ...
    return [img,rad, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c]
def collate_MIL_survival_p(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    label = torch.LongTensor([item[1] for item in batch])
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    ...
    if mode == 'coattn':
        ...
        collate = collate_MIL_survival_p
    elif mode == 'rpgattn':
        collate = collate_MIL_survival_rpg
    else:
...
# def nll_loss(hazards, S, Y, c, alpha=0.4, eps=1e-7, device='cpu'):
#     batch_size = len(Y)
#     Y = Y.view(batch_size, 1).to(torch.int64).to(device)  # ground truth bin, 1,2,...,k
#     c = c.view(batch_size, 1).float().to(device)  # censorship status, 0 or 1

#     if S is None:
#         S = torch.cumprod(1 - hazards, dim=1).to(device)  # survival is cumulative product of 1 - hazards

#     # without padding, S(0) = S[0], h(0) = h[0]
#     S_padded = torch.cat([torch.ones((batch_size, 1)).to(device), S], 1)  # S(-1) = 0, all patients are alive from (-inf, 0) by definition
...
def get_custom_exp_code(args):
    ...
    if args.early_stopping:
      param_code += '_early_stopping' 

    if args.max_epochs != 20:
      param_code += '_epoch%s' %str(args.max_epochs)
