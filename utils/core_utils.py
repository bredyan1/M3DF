...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.model_type =='snn':
        model_dict = { 'model_size_omic': args.model_size_omic, 'n_classes': args.n_classes}
...
def train_loop_survival(epoch, model, train_loader, val_loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):   
    ...
    for batch_idx, (data_WSI, x_omic1,x_omic2,x_omic3,x_omic4,x_omic5,x_omic6,label, event_time, c) in enumerate(train_loader):
        data_omic = {}
    ...
    with torch.no_grad():
        for batch_idx, (data_WSI, x_omic1,x_omic2,x_omic3,x_omic4,x_omic5,x_omic6,label, event_time, c) in enumerate(val_loader):
            data_omic = {}
            data_omic['x_omic1'] = x_omic1.to(device)
            data_omic['x_omic2'] = x_omic2.to(device)
            data_omic['x_omic3'] = x_omic3.to(device)                                        
...
def validate_and_plot(val_loader, model, loss_fn, device, epoch, writer=None,lambda_reg=0.,  reg_fn=None):
    ...
    #         # 组织输入数据
    #         data_omic = {
    #             'x_omic1': x_omic1.to(device),
    #             'x_omic2': x_omic2.to(device),
    #             'x_omic3': x_omic3.to(device),
    #             'x_omic4': x_omic4.to(device),
    #             'x_omic5': x_omic5.to(device),
    #             'x_omic6': x_omic6.to(device),
    #         }
    #         data_WSI = data_WSI.to(device)
    #         label, c, event_time = label.to(device), c.to(device), event_time.to(device)

    #         # 模型前向传播
    #         hazards, S, Y_hat, _ = model(
    #             x_path=data_WSI,
    #             x_omic1=data_omic['x_omic1'], x_omic2=data_omic['x_omic2'], 
    #             x_omic3=data_omic['x_omic3'], x_omic4=data_omic['x_omic4'],
    #             x_omic5=data_omic['x_omic5'], x_omic6=data_omic['x_omic6']
    #         )
            ...
            # **从 S 计算预测的生存时间**
            time_bins = torch.arange(1, S.shape[1] + 1).float().to(device)  # 假设类别索引代表时间间隔
            predicted_times = torch.sum(S * time_bins, dim=1)  # 计算预测的生存时间
    ...
    print(f'Epoch: {epoch}, val_loss_surv: {val_loss_surv:.4f}, val_loss: {val_loss:.4f}, val_c_index: {val_c_index:.4f}')
    ...
    # 设置初始文件名
    kmf_plot_path = f"/home/yanyiqun/MCAT/km0/kmf_epoch_mcat_{epoch}_.png"

    # 如果文件已存在，添加递增编号
    file_index = 1
    while os.path.exists(kmf_plot_path):
        kmf_plot_path = f"/home/yanyiqun/MCAT/km0/kmf_epoch_mcat_{epoch}_{file_index}.png"
        file_index += 1
