...
from models.model_pr import PRCAT_Surv
from models.model_rpg import RPGCAT_Surv
from models.model_rpg1 import RPGCAT_Surv1
from models.model_rpg2 import RPGCAT_Surv2
from utils.utils import *

from utils.coattn_train_utils_pr import *
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.model_type =='snn':
        ...
        model_dict = { 'fusion': args.fusion, 'n_classes': args.n_classes}
        model = MIL_Attention_FC_surv(**model_dict)
    ...
    elif args.model_type == 'prcat':
        model_dict = {'fusion': args.fusion, 'n_classes': args.n_classes}
        model = PRCAT_Surv(**model_dict)
    elif args.model_type == 'rpgcat':
        model_dict = {'fusion': args.fusion, 'omic_sizes': args.omic_sizes,'n_classes': args.n_classes}
        model = RPGCAT_Surv(**model_dict)
    elif args.model_type == 'rpgcat1':
        model_dict = {'fusion': args.fusion, 'omic_sizes': args.omic_sizes,'n_classes': args.n_classes}
        model = RPGCAT_Surv1(**model_dict)
    elif args.model_type == 'rpgcat2':
        model_dict = {'fusion': args.fusion, 'omic_sizes': args.omic_sizes,'n_classes': args.n_classes}
        model = RPGCAT_Surv2(**model_dict)
    else:
        raise NotImplementedError
    ...
    for epoch in range(args.max_epochs):
        ...
        #     stop = validate_survival_coattn(cur, epoch, model, val_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir)
        # else:
        train_loop_survival(epoch, model, train_loader,val_loader, optimizer, args.n_classes, writer, loss_fn, reg_fn, args.lambda_reg, args.gc)
        # validate_and_plot(val_loader, model, loss_fn, device, epoch, writer=None,lambda_reg=0.,  reg_fn=None)
        print("best_val_c_index is {:.4f}".format(best_val_c_index))
        #last_layer_params = {k: v for k, v in best_params.items() if k.startswith('classifier.')}
    ...
    torch.save(best_params, os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
    loaded_model_params = torch.load(os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
    model.load_state_dict(loaded_model_params)
    results_val_dict, val_cindex = summary_survival(model, val_loader, args.n_classes)
    val_cindex = best_val_c_index
...
def train_loop_survival(epoch, model, train_loader, val_loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):   
    global best_val_c_index
    global best_params
    ...
    for batch_idx, (data_WSI, data_rad,x_omic1,x_omic2,x_omic3,x_omic4,x_omic5,x_omic6,label, event_time, c) in enumerate(train_loader):
        data_omic = {}
        ...
        data_WSI, data_rad = data_WSI.to(device), data_rad.to(device)
        
        #data_WSI, data_omic = data_WSI.to(device), data_omic.to(device)
        label = label.to(device)
        c = c.to(device)
        hazards, S, Y_hat, _,h = model(x_path=data_WSI,x_rad=data_rad,  x_omic1=data_omic['x_omic1'], x_omic2=data_omic['x_omic2'], x_omic3=data_omic['x_omic3'], x_omic4=data_omic['x_omic4'],x_omic5=data_omic['x_omic5'], x_omic6=data_omic['x_omic6'],)
        #hazards, S, Y_hat, _ = model(x_path=data_WSI, x_omic=data_omic) # return hazards, S, Y_hat, A_raw, results_dict
    ...
    with torch.no_grad():
        for batch_idx, (data_WSI,data_rad, x_omic1,x_omic2,x_omic3,x_omic4,x_omic5,x_omic6,label, event_time, c) in enumerate(val_loader):
            data_omic = {}
            data_omic['x_omic1'] = x_omic1.to(device)
            data_omic['x_omic2'] = x_omic2.to(device)
            data_omic['x_omic3'] = x_omic3.to(device)                                        
    ...
    print('Epoch: {}, val_loss_surv: {:.4f}, val_loss: {:.4f}, val_c_index: {:.4f}'.format(epoch, val_loss_surv, val_loss, val_c_index))

    if writer:
        writer.add_scalar('val/loss_surv', val_loss_surv, epoch)
        writer.add_scalar('val/loss', val_loss, epoch)
        writer.add_scalar('val/c_index', val_c_index, epoch)
    
    # 保存模型参数

    print(f"Current best_val_c_index: {best_val_c_index}, Current epoch val_c_index: {val_c_index}")
...
def validate_and_plot(val_loader, model, loss_fn, device, epoch, writer=None,lambda_reg=0.,  reg_fn=None):
    # 验证
    model.eval()
    val_loss_surv, val_loss = 0., 0.
    all_risk_scores = np.zeros((len(val_loader)))
    ...
    with torch.no_grad():
        for batch_idx, (data_WSI,data_rad, x_omic1,x_omic2,x_omic3,x_omic4,x_omic5,x_omic6,label, event_time, c) in enumerate(val_loader):
            ...
            val_loss += loss_value + loss_reg
                        # 生成保存路径（与KM图路径保持一致）
            # h_save_path = f"/home/yanyiqun/MCAT/tsne/h/h_epoch_{epoch}_{batch_idx}_{label}.pt"
            
            # # 处理文件名冲突
            # file_index = 1
            # while os.path.exists(h_save_path):
            #     h_save_path = f"/home/yanyiqun/MCAT/tsne/h/h_epoch_{epoch}_{batch_idx}_{file_index}_{label}.pt"
            #     file_index += 1
            
            # # 保存为PyTorch张量文件
            # torch.save(h, h_save_path)  # 
            # print(f"Epoch {epoch}: Hidden features 'h' saved as {h_save_path}")
    ...
    # high_risk_mask = np.array(all_risk_scores) >= median_risk
    # low_risk_mask = np.array(all_risk_scores) < median_risk

    # # **绘制高风险组**
    # kmf.fit(durations=np.array(all_event_times)[high_risk_mask], event_observed=np.array(all_censorships)[high_risk_mask])
    # kmf.plot_survival_function(label="Low Risk")

    # # **绘制低风险组**
    # kmf.fit(durations=np.array(all_event_times)[low_risk_mask], event_observed=np.array(all_censorships)[low_risk_mask])
    # kmf.plot_survival_function(label="High Risk")

    # plt.title(f"Kaplan-Meier Curve (Epoch {epoch})")
    # plt.xlabel("Time")
    # plt.ylabel("Survival Probability")
    # plt.legend()
    # plt.grid()

    # # 计算 Log-rank 检验的 p-value
    # results = logrank_test(
    #     np.array(all_event_times)[high_risk_mask], np.array(all_event_times)[low_risk_mask],
    #     event_observed_A=np.array(all_censorships)[high_risk_mask],
    #     event_observed_B=np.array(all_censorships)[low_risk_mask]
    # )
    # p_value = results.p_value
    # print(f"Log-rank Test p-value: {p_value:.4f}")

    # # 显示 p-value
    # plt.text(0.5, 0.1, f'p = {p_value:.4f}', transform=plt.gca().transAxes, fontsize=12, verticalalignment='bottom')

    # import os

    # # 设置初始文件名
    # kmf_plot_path = f"/home/yanyiqun/MCAT/km0/kmf_epoch_rpgcat_{epoch}_.png"

    # # 如果文件已存在，添加递增编号
    # file_index = 1
    # while os.path.exists(kmf_plot_path):
    #     kmf_plot_path = f"/home/yanyiqun/MCAT/km0/kmf_epoch_rpgcat_{epoch}_{file_index}.png"
    #     file_index += 1

    # plt.savefig(kmf_plot_path)
    # plt.close()

    # if writer:
    #     writer.add_figure(f'Kaplan-Meier Curve (Epoch {epoch})', plt.gcf(), epoch)

    # print(f"Epoch {epoch}: Kaplan-Meier plot saved as {kmf_plot_path}")
...
#         #hazards, S, Y_hat, _ = model(x_path=data_WSI,  x_omic1=data_omic['x_omic1'], x_omic2=data_omic['x_omic2'], x_omic3=data_omic['x_omic3'], x_omic4=data_omic['x_omic4'],x_omic5=data_omic['x_omic5'], x_omic6=data_omic['x_omic6'],)
#         hazards, S, Y_hat, _ = model(x_path=data_WSI, x_omic=data_omic) # return hazards, S, Y_hat, A_raw, results_dict
...
def validate_survival(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    ...
    for batch_idx, (data_WSI,data_rad, x_omic1,x_omic2,x_omic3,x_omic4,x_omic5,x_omic6, label, event_time, c) in enumerate(loader):
        data_omic = {}
        data_omic['x_omic1'] = x_omic1.to(device)
        data_omic['x_omic2'] = x_omic2.to(device)
        data_omic['x_omic3'] = x_omic3.to(device)                                        
