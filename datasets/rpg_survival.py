...
class Generic_MIL_Survival_Dataset_rpg(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir,rad_data_dir,OOM=4096, mode: str='omic', **kwargs):
        super(Generic_MIL_Survival_Dataset_rpg, self).__init__(**kwargs)
    ...
    def __getitem__(self, idx):
        ...
        if type(self.rad_data_dir) == dict:
            source = self.slide_data['oncotree_code'][idx]
            rad_data_dir = self.rad_data_dir[source]
        else:
            rad_data_dir = self.rad_data_dir
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    rad_features = []
                    for slide_id in slide_ids:
                        #rad_path = os.path.join('/mnt/sdb/yanyiqun/TCGA_ROOT_DIR/tcga_gbmlgg/tcga_gbmlgg_mri_features_resnet50',  '{}.pt'.format(slide_id[:12].rstrip('.svs')))
                        rad_path = os.path.join('/mnt/sdb/yanyiqun/TCGA_ROOT_DIR/tcga_gbmlgg/tcga_gbmlgg_mri_features',  '{}.pt'.format(slide_id[:12].rstrip('.svs')))
                        # rad_path = os.path.join('/mnt/sdb/yanyiqun/TCGA_ROOT_DIR/tcga_brca/tcga_brca_mri_features',  '{}.pt'.format(slide_id[:12].rstrip('.svs')))
                 
                        rad_bag = torch.load(rad_path, map_location='cpu')
                        rad_features.append(rad_bag)
                    rad_features = torch.cat(rad_features, dim=0)
                    omic1 = torch.tensor(self.genomic_features[self.omic_names[0]].iloc[idx])
                    omic2 = torch.tensor(self.genomic_features[self.omic_names[1]].iloc[idx])
                    omic3 = torch.tensor(self.genomic_features[self.omic_names[2]].iloc[idx])
                    ...
                    omic5 = torch.tensor(self.genomic_features[self.omic_names[4]].iloc[idx])
                    omic6 = torch.tensor(self.genomic_features[self.omic_names[5]].iloc[idx])
                    return (path_features,rad_features, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c)
                else:
...
class Generic_Split(Generic_MIL_Survival_Dataset_rpg):
    def __init__(self, slide_data, metadata, mode, signatures=None, data_dir=None, rad_data_dir=None ,label_col=None, patient_dict=None, num_classes=2):
        self.use_h5 = False
        self.slide_data = slide_data
