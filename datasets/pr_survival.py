...
class Generic_MIL_Survival_Dataset_pr(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir,rad_data_dir,OOM=4096, mode: str='omic', **kwargs):
        super(Generic_MIL_Survival_Dataset_pr, self).__init__(**kwargs)
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    return (path_features, rad_features, label, event_time, c)
                elif self.mode == 'prattn':
                    path_features = []
                    for slide_id in slide_ids:
                        wsi_path = os.path.join(data_dir,  '{}.pt'.format(slide_id.rstrip('.svs')))
                        wsi_bag = torch.load(wsi_path)
...
class Generic_Split(Generic_MIL_Survival_Dataset_pr):
