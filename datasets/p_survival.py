...
class Generic_MIL_Survival_Dataset_p(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir, mode: str='omic', **kwargs):
        super(Generic_MIL_Survival_Dataset_p, self).__init__(**kwargs)
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    rad_features = []
                    for slide_id in slide_ids:
                        rad_path = os.path.join('/mnt/sdb/yanyiqun/TCGA_ROOT_DIR/tcga_gbmlgg/tcga_gbmlgg_mri_features_resnet152',  '{}.pt'.format(slide_id[:12].rstrip('.svs')))
                        rad_bag = torch.load(rad_path)
                        rad_features.append(rad_bag)
                    rad_features = torch.cat(rad_features, dim=0)
                    return (path_features, rad_features, label, event_time, c)
                elif self.mode == 'pattn':
                    path_features = []
                    for slide_id in slide_ids:
                        wsi_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id.rstrip('.svs')))
                        wsi_bag = torch.load(wsi_path)
...
class Generic_Split(Generic_MIL_Survival_Dataset_p):
