...
### Internal Imports
from datasets.rpg_survival import Generic_WSI_Survival_Dataset, Generic_MIL_Survival_Dataset_rpg
from utils.file_utils import save_pkl, load_pkl
from utils.core_utils_rpg import train
...
def main(args):
	...
	for i in folds:
		...
		elif 'rpgattn' in args.mode:
			args.omic_sizes = train_dataset.omic_sizes
			print('Genomic Dimensions', args.omic_sizes)
		elif 'prattn' in args.mode: 
...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['snn', 'deepset', 'amil', 'mi_fcn', 'mcat','prcat','rpgcat'], default='mcat', help='Type of model (Default: mcat)')
parser.add_argument('--mode',            type=str, choices=['omic', 'path', 'pathomic', 'cluster', 'coattn','prattn','rpgattn'], default='coattn', help='Specifies which modalities to use / collate function in dataloader.')
parser.add_argument('--fusion',          type=str, choices=['None', 'concat', 'bilinear','gate'], default='concat', help='Type of fusion. (Default: concat).')
parser.add_argument('--apply_sig',		 action='store_true', default=False, help='Use features as signature embeddings.')
...
parser.add_argument('--model_size_rad', type=str, default='small', help='Network size of SNN model')
...
settings = {'num_splits': args.k, 
			...
			'model_size_rad': args.model_size_rad,
...
if 'survival' in args.task:
	...
	rad_dir='%s_mri_features' % combined_study
	dataset = Generic_MIL_Survival_Dataset_rpg(csv_path = './%s/%s_all_clean.csv' % (args.dataset_path, combined_study),
										   mode = args.mode,
										   apply_sig = args.apply_sig,
										   data_dir= os.path.join(args.data_root_dir, study_dir),
										   rad_data_dir= os.path.join(args.data_root_dir, rad_dir),
                                           shuffle = False, 
										   seed = args.seed, 
										   print_info = True,
										   patient_strat= False,
...
if isinstance(dataset, Generic_MIL_Survival_Dataset_rpg):
...
results_dir = os.path.join(
    ...
    args.which_splits,
    args.param_code,
    f"{args.exp_code}_s{args.seed}"  # 原种子后缀，例如 "_s0"
)
if not os.path.isdir(args.results_dir):
	os.makedirs(args.results_dir)
# 检查路径是否存在，若存在则添加递增后缀
...
while os.path.exists(temp_dir):
    temp_dir = f"{results_dir}_{suffix_idx}"
    suffix_idx += 1

# 更新最终结果路径并创建目录
args.results_dir = temp_dir
os.makedirs(args.results_dir)
